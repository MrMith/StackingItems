...
namespace StackingItems
{
	...
	class StackMain : Plugin
	{
		...
		public override void Register()
		{
			...
			this.AddConfig(new Smod2.Config.ConfigSetting("si_914enable", true, Smod2.Config.SettingType.BOOL, true, "Should player's inventory interact with 914. Set to false to disable any interaction with 914 (Items will be removed if disabled)"));
			this.AddConfig(new Smod2.Config.ConfigSetting("si_914handorinv", 0, Smod2.Config.SettingType.NUMERIC, true, "Integer value that decides wether to not touch the inventory and just pass items along or upgrade item in hand or the whole user's inventory. 0 = Transfer items no upgrades, 1 = Entire inventory gets upgraded, 2 = just hand gets upgraded."));
			this.AddConfig(new Smod2.Config.ConfigSetting("si_914itemstosave", new int[] { 12,14,15,17,18,19 }, Smod2.Config.SettingType.NUMERIC_LIST, true, "Any itemtype in this list will NOT be consumed by 914 while in inventory or hand."));
		}
	}
}